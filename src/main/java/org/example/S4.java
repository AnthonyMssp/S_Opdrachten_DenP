-- ------------------------------------------------------------------------
        ...
        SELECT AGE(MAX(gbdatum), MIN(gbdatum)) AS verschil, AVG(AGE(gbdatum)) AS gemiddelde FROM medewerkers;
        ...
        SUM(CASE WHEN functie = 'VERKOPER' THEN comm ELSE 0 END) / COUNT(CASE WHEN functie = 'VERKOPER' THEN 1 ELSE NULL END) AS commissie_verkopers
        FROM medewerkers;
