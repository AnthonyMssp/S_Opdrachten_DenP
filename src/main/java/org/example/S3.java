-- ------------------------------------------------------------------------
        ...
        JOIN medewerkers ON docent = mnr JOIN cursussen ON cursus = code;

        -- S3.2.
        -- Geef in twee kolommen naast elkaar de achternaam van elke cursist (`cursist`)
        -- van alle S02-cursussen, met de achternaam van zijn cursusdocent (`docent`).
        DROP VIEW IF EXISTS s3_2; CREATE OR REPLACE VIEW s3_2 AS                                                     -- [TEST]
        SELECT cursist.naam AS cursist, docent.naam AS docent FROM inschrijvingen i
        INNER JOIN medewerkers cursist ON cursist = mnr
        INNER JOIN uitvoeringen u ON u.begindatum = i.begindatum AND u.cursus = i.cursus
        INNER JOIN medewerkers docent ON u.docent= docent.mnr
        WHERE i.cursus = 'S02';
        ...
        SELECT afdelingen.naam, medewerkers.naam AS hoofd FROM afdelingen
        JOIN medewerkers ON hoofd = mnr;

        -- S3.4.
        -- Geef de namen van alle medewerkers, de naam van hun afdeling (`afdeling`)
        -- en de bijbehorende locatie.
        DROP VIEW IF EXISTS s3_4; CREATE OR REPLACE VIEW s3_4 AS                                                     -- [TEST]
        SELECT medewerkers.naam, afdelingen.naam AS afdeling, locatie FROM medewerkers
        JOIN afdelingen ON afd = anr;
        ...
        JOIN inschrijvingen ON cursist = mnr
        WHERE cursus = 'S02' AND inschrijvingen.begindatum = '2019-04-12';
